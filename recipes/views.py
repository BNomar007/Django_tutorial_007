...
def recipe_delete_view(request, id=None):
    ...
    if obj is None:
        if request.htmx:
            return HttpResponse('Noot found')
        raise Http404
...
def recipe_ingredient_delete_view(request, parent_id=None, id=None):
    ...
    if request.method == 'POST':
        ...
        if request.htmx:
            #return HttpResponse("<span style='color: #007500';>Ingredient deleted.</span>")
...
def recipe_detail_hx_view(request, id=None):
    ...
    if obj is None:
        return HttpResponse('The object you are looking for does not exist.')
    context = {
        'object': obj
    }
